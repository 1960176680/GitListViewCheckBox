...
//		listener.onCheckedChange((Integer) v.getTag());
		...
		if (getItem((Integer) v.getTag()) instanceof Model) {
			if(((Model) getItem((Integer) v.getTag())).isChecked()){
				((Model) getItem((Integer) v.getTag())).setChecked(false);
			}else{
				((Model) getItem((Integer) v.getTag())).setChecked(true);
			};
